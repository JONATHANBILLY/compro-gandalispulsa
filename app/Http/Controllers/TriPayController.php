...
class TriPayController extends Controller
{
    ...
    public function testedRequest($jumlah)
    {
        $faker = Factory::create('id_ID');
        $merchant_ref = Str::random(6);
        $api = \DB::table('setting_webs')->where('id', 1)->first();
        $data = [
            'method' => 'QRIS',
            'merchant_ref' => 'TEST_' . $merchant_ref,
            'amount' => $jumlah,
            'customer_name' => env("APP_NAME"),
            'customer_email' => $merchant_ref . '@gamingku.net',
            'customer_phone' => $faker->phoneNumber(),
            'order_items' => [
                [
                    'name' => 'Test Local ' . $merchant_ref,
                    'price' => $jumlah,
                    'quantity' => 1,
                ]
            ],
            'callback_url' => env('APP_URL') . '/callback',
            'return_url' => env('APP_URL') . '/pembelian/invoice/' . $merchant_ref,
            'expired_time' => (time() + (24 * 60 * 60)), // 24 jam
            'signature' => hash_hmac('sha256', $api->tripay_merchant_code . $merchant_ref . $jumlah, $api->tripay_private_key)
        ];
