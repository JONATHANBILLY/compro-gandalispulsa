...
class OrderController extends Controller
{
    ...
    {
        $data = Kategori::where('kode', $kategori->kode)->select('nama', 'server_id', 'thumbnail', 'id', 'kode', 'petunjuk', 'placeholder_1', 'placeholder_2', 'deskripsi_game', 'deskripsi_field', 'banner', 'tipe')->first();
        ...
        $methods = \App\Models\Method::enabledMethods();
    ...
    public function confirm(Request $request)
    {
        ...
        $digicheck = new digiFlazzController();
...
//            , 'hago', 'higgs-domino', 'speed-drifters', 'ragnarok-x-next-generation', 'laplace-m', 'league-of-legends', 'auto-chess' ,'lost-saga-origin', 'pln', 'dana','ovo','shopeepay','Gopay','linkaja'];
        $daftarGameValidasi = ['disable-cekid'];
        if (in_array($dataKategori->kode, $daftarGameValidasi)) {
            ...
            } elseif ($dataKategori->kode == "pln") {
                $data = $digicheck->cekpln($request->uid);
            } elseif ($dataKategori->kode == "dana") {
                $data = $apicheck->check_bank($request->uid, 'DANA');
            } elseif ($dataKategori->kode == "ovo") {
                $data = $apicheck->check_bank($request->uid, 'OVO');
            } elseif ($dataKategori->kode == "shopeepay") {
                $data = $apicheck->check_bank($request->uid, 'SHOPEEPAY');
            } elseif ($dataKategori->kode == "Gopay") {
                $data = $apicheck->check_bank($request->uid, 'GOPAY');
            } elseif ($dataKategori->kode == "linkaja") {
                $data = $apicheck->check_bank($request->uid, 'LINKAJA');
            }
            if ($data['status']['code'] == 1) {
                return response()->json([
                    'status' => false,
                    'data' => isset($data['status']['message']) ? $data['status']['message'] : 'Username tidak ditemukan atau coba lagi nanti'
                ]);
            }
    ...
    public function store(Request $request)
    {
        ...
        $order_id = 'INV' . now()->format('YmdHisu') . Str::random(6);
        $tripay = new TriPayController();
        $cronos = new CronosController();
        ...
        if ($request->payment_method == "SALDO") {
        ...
        } else if($request->payment_method == "BANK-TRANSFER"){
            $amount = $dataLayanan->harga;
        } else if ($request->payment_method == "OVO_MANUAL" || $request->payment_method == "GOPAY_MANUAL") {
            ...
            //     return response()->json([
            //         'status' => false,
            //         'data' => "Mohon maaf, tidak dapat menggunakan metode pembayaran ini!"
            //     ]);

            // }

            if ($request->payment_method == "QRISC") {

                $cronosRes = $cronos->requestQris($order_id, $dataLayanan->harga, $request->payment_method, $request->email, $request->nomor);
            } else if(
                ...
                $cronosRes = $cronos->requestVirtualAccount($order_id, $dataLayanan->harga, $request->payment_method, $request->email, $request->nomor);
    
            } else {
                dd('die');
            }
            
            if ($cronosRes['success'] != true) {
                return response()->json(['status' => false, 'data' => 'Request pembayaran gagal. Silahkan kontak CS. Terima kasih']);
            }

            $no_pembayaran = $cronosRes['no_pembayaran'];
            $reference = $cronosRes['reference'];
            $amount = $cronosRes['amount'];
        ...
        if ($request->payment_method != "SALDO") {
            ...
            if (Auth::check()) {
                $sess_username = Auth::user()->username;
            } else {
                $sess_username = '';
            }

            $pembelian = new Pembelian();
            $pembelian->username = $sess_username;
            ...
            $pembelian->keterangan = 'Sedang diproses...';
            ...
            $provider_order_id = 'ORDER' . now()->format('YmdHisu') . Str::random(6);
